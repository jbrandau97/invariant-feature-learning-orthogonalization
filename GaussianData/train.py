...
from models import *
from utils import generate_data, binary_acc, gkern
...
parser.add_argument('--model_dir', type=str, default='experiments/final/cf2/', help='directory name to store output')
...
parser.add_argument('--cf_dim', type=int, default=1, help='dimension of confounder')
parser.add_argument('--num_features', type=int, default=32, help='number of features in SSN')
...
def run_experiment(mdn, 
    ...
    skmetrics_f_file = os.path.join(experiment_name, 'skmetrics_f.txt')
    ...
    if mdn == 'Baseline':
        ...
        model = SSN(batch_size=batch_size, cf_dim=cf_dim, num_features=num_features)
    else:
    ...
    for e in range(epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            if mdn == 'SSN':
                with torch.no_grad():
                    if cf_dim == 1:
                        model.cfs = nn.Parameter(torch.Tensor(cf_batch)[:,None].to(device), requires_grad=False)
                    elif cf_dim == 2:
                        model.cfs = nn.Parameter(torch.cat((torch.ones(len(cf_batch), 1), torch.Tensor(cf_batch)[:,None]), dim=1).to(device), requires_grad=False)
                    else:
            ...
            # Forward pass
            optimizer.zero_grad()
            y_pred, fc = model(data, output_fc=True)
            loss = criterion(y_pred, target.unsqueeze(1)) #+ 0.1*torch.norm(model.cfs @ torch.inverse(model.cfs.T @ model.cfs) @ model.cfs.T @ ssn_fc)
        ...
        # Calculation of delta parameter for SSN
        if mdn == 'SSN':
            ...
            for sample_batched in train_loader:
                ...
                with torch.no_grad():
                    if cf_dim == 1:
                        ...
                        raise ValueError("cf_dim must be 1 or 2")
                features = model.fc2(F.relu(model.fc1(model.model_conv(data).view(-1, 18432))))
                XTX += model.cfs.T @ model.cfs
                XTH += model.cfs.T @ features
            model.delta = nn.Parameter(torch.inverse(XTX) @ XTH, requires_grad=False)
        ...
        if epoch_loss_val + 0.001 < min_loss:
            ...
            if run <= 5:
                torch.save(model.state_dict(), os.path.join('plot_data', run_name, 'best_model.pth'))
            patience = 0
    ...
    y_test, y_pred_list, y_test_f, y_pred_list_f = test(model, mdn, batch_size, cf_dim, run=run, N=N)
    ...
    with open(skmetrics_f_file, 'a') as f:
        f.write("Report for run " + str(run))
        f.write(classification_report(y_test_f, y_pred_list_f, digits=3) + '\n')

# Test model
def test(model, mdn, batch_size, cf_dim, run=None, N=2000):
    ...
    y_test_f, cf_test_f, cf2_test_f, mf_test_f, x_test_f, y_test_f = generate_data(N, True, seed=args.seed+3)
    X_test = torch.from_numpy(np.swapaxes(x_test, 1, 3)).float()
    X_test_f = torch.from_numpy(np.swapaxes(x_test_f, 1, 3)).float()
    test_set = SyntheticDataset(X_test, y_test, cf_test)
    test_set_f = SyntheticDataset(X_test_f, y_test_f, cf_test_f)
    ...
    with torch.no_grad():
        for i, sample_batched in enumerate(test_loader):
            ...
            if mdn == 'SSN':
                if cf_dim == 1:
                        model.cfs = nn.Parameter(torch.Tensor(cf_batch)[:,None], requires_grad=False)
                elif cf_dim == 2:
                    model.cfs = nn.Parameter(torch.cat((torch.ones(len(cf_batch), 1), torch.Tensor(cf_batch)[:,None]), dim=1), requires_grad=False)
                else:
                    raise ValueError("cf_dim must be 1 or 2")
            elif mdn == 'Baseline':
                None
            ...
            y_test_pred = model(data, output_fc=False)
            y_pred_tag = torch.round(y_test_pred)
            y_pred_list.append(y_pred_tag.cpu())
            
    y_pred_list_f = []
    with torch.no_grad():
        for i, sample_batched in enumerate(test_loader_f):
            data = sample_batched['image'].float()
            target = sample_batched['label'].float()
            cf_batch = sample_batched['cfs'].float()
            ...
            y_test_pred_f = model(data, output_fc=False)
            y_pred_tag_f = torch.round(y_test_pred_f)
            y_pred_list_f.append(y_pred_tag_f.cpu())

    # Create evaluation
    y_pred_list = np.array([a.squeeze().tolist() for a in y_pred_list]).flatten()
    y_pred_list_f = np.array([a.squeeze().tolist() for a in y_pred_list_f]).flatten()
    print(classification_report(y_test, y_pred_list, digits=3))
    print(confusion_matrix(y_test, y_pred_list)) 
    return y_test, y_pred_list, y_test_f, y_pred_list_f
