...
def train_and_validate(model, train_loader, val_loader, device, epochs, lr_beta, lr_model):
    ...
    # Save final model
    torch.save(model.state_dict(), f'final_model_{batch_size}.pth')
    return final_model
...
if __name__ == '__main__':
    ...
    labels_test_f, cf1_test_f, _, _, x_test_f, y_test_f = generate_data(N, flip=True, seed=4)
    x_new = np.swapaxes(x, 1, 3)
    x_new_val = np.swapaxes(x_val, 1, 3)
    x_new_test = np.swapaxes(x_test, 1, 3)
    x_new_test_f = np.swapaxes(x_test, 1, 3)
    ...
    test_set_f = SyntheticDataset(x_new_test_f, labels_test_f, cf1_test_f)
    test_loader_f = torch.utils.data.DataLoader(
        test_set_f, batch_size=20, shuffle=True, pin_memory=True)

    f0_corr, f1_corr, acc = 0, 0, 0
    f0_corr_f, f1_corr_f, acc_f = 0, 0, 0
    for _ in range(num_runs):
        ...
        acc += acc_temp
        print("Test acc. (A):" + str(acc_temp))
        f0_corr_temp_f, f1_corr_temp_f, acc_temp_f = test(final_model, test_loader_f, device)
        f0_corr_f += f0_corr_temp_f
        f1_corr_f += f1_corr_temp_f
        acc_f += acc_temp_f
        print("Test acc. (B):" + str(acc_temp_f))

    print("Overall Correlations:", f0_corr / num_runs, f1_corr / num_runs)
    print("Overall Accuracy:", acc / num_runs)
    print("Overall Correlations f:", f0_corr_f / num_runs, f1_corr_f / num_runs)
    print("Overall Accuracy f:", acc_f / num_runs)